...
This project has been entirely generated by AI, I was playing the role of a "supervisor".
The goal is to show how AI can help building software, not only with application code but also with the tooling around each project: tests, toolchain, local environments, documentation, analysis, etc.

I haven't written any line of code or documentation (except this lines included in the `final note` commit), and as illustrated in the analysis table at the bottom of this file, I haven't put rigid rules in place. I also want to mention that it's my first attempt on a "full" project coded by AI.
The things I did not ask AI to do: create a venv, install Rancher Desktop and K3S, install claude itself (obviously)
...
The project cost is ~**$8** with `Claude-3.7-Sonnet`, using `Claude Code`, and a few other knowledge requests on the Claude app.

### AI requests' cost analysis

This table represents a set of all the requests that were asked to `Claude Code`
